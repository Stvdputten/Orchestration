...
clean_up(){
	...
	ssh -n $remote "sudo kill -9 \$(ps -ef | grep 5000 | head -n 1 | awk '{ print \$2 }')" > /dev/null 2>&1
	ssh -n $remote "sudo kill -9 \$(lsof -i :8080 | head -n 2 | awk 'NR==2 { print \$2 }')" > /dev/null 2>&1
	ssh -n $remote "sudo kill -9 \$(ps -ef | grep 16686 | head -n 1 | awk '{ print \$2 }')" > /dev/null 2>&1
