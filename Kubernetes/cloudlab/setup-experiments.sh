...
# pssh -i -h $ips "cd DeathStarBench && git pull && git reset --hard origin" >/dev/null 2>&1
# ssh -n $remote "cd DeathStarBench && git reset --hard origin && git pull" >/dev/null 2>&1
...
run_benchmark() {
	if [ $benchmark == "hotelReservation" ]; then
		...
		# ssh -n $manager "kubectl get all -A"  | grep deployment.apps/nginx | grep 1/1 > /dev/null 2>&1
		#
		# TODO change the check , if it has a cluster_ip_frontend than it is deployed
		...
		if [ $? -ne 0 ]; then
			...
			# PORT=8080
			# PID=$(ssh $remote "sudo lsof -t -i:$PORT")

			# if [ -n "$PID" ]; then
			# 	echo "Port $PORT is occupied by process ID $PID. Killing the process..."
			# 	sudo kill -9 $PID
