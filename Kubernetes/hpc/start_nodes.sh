...
ready=$(onevm list -f NAME~k8s -l STAT --no-header | grep "runn" | wc -l)
while [ ! $ready -eq 0 ]; do
  sleep 5
  ready=$(onevm list -f NAME~k8s -l STAT --no-header | grep "runn" | wc -l)
...
echo "Starting k8 cluster with $managers managers and $workers workers."
# exit 1

i=1
while [ "$i" -le "$managers" ]; do
    onetemplate instantiate ${template} --name "k8s-manager-${i}"
    # echo "$i"
    i=$(($i + 1))
done
...
while [ "$i" -le "$workers" ]; do
    onetemplate instantiate ${template} --name "k8s-worker-${i}"
    # echo "$i"
    i=$(($i + 1))
done
