...
do
	...
	for benchmark in socialNetwork 
	do
		if [ $benchmark == "hotelReservation" ]; then
			...
			ssh -n $remote "cd DeathStarBench/hotelReservation/wrk2 && export nginx_ip=$node_name_nginx && ./workload.sh" > ./results/$(echo $remote | cut -d@ -f2)-swarm-hr-wrk-mixed.txt
			...
			ssh -n $remote "cd DeathStarBench/mediaMicroservices/wrk2 && export nginx_ip=$node_name_nginx && ./workload.sh" > ./results/$(echo $remote | cut -d@ -f2)-swarm-mm-wrk-compose.txt
			...
			ssh -n $remote "cd DeathStarBench/socialNetwork/wrk2 && export nginx_ip=$node_name_nginx && ./workload-home.sh" > ./results/$(echo $remote | cut -d@ -f2)-swarm-sn-wrk-home.txt
			ssh -n $remote "cd DeathStarBench/socialNetwork/wrk2 && export nginx_ip=$node_name_nginx && ./workload-user.sh" > ./results/$(echo $remote | cut -d@ -f2)-swarm-sn-wrk-user.txt
			ssh -n $remote "cd DeathStarBench/socialNetwork/wrk2 && export nginx_ip=$node_name_nginx && ./workload-compose.sh" > ./results/$(echo $remote | cut -d@ -f2)-swarm-sn-wrk-compose.txt
