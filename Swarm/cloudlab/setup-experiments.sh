...
echo "Client is number $clients and $remote"
...
run_benchmark(){
	if [ $benchmark == "hotelReservation" ]; then
		...
		nginx_ip=10.10.1.$(($(ssh stvdp@$node_name_nginx "hostname | cut -d'.' -f1 | sed 's/node//'")+1))
		node_name_jaeger=$(ssh $manager "docker service ps hotel-reservation_jaeger | awk 'FNR == 2 {print \$4}'")
		echo "The nginx service is found on $node_name_nginx, $nginx_ip"
		...
		ssh -n $remote "cd DeathStarBench/hotelReservation/wrk2 && export nginx_ip=$nginx_ip && ./workload.sh -t $t -c $c -d $d -R $R" > ./results/$dir_date/swarm-hr-wrk-mixed-$output_name
		...
		if [ $? -ne 0 ]; then
			...
			echo "$bench_name has been deployed"
		else 
			echo "Benchmark already deployed"
		fi
		...
		ssh -n $remote "cd DeathStarBench/mediaMicroservices/wrk2 && export nginx_ip=$nginx_ip && ./workload.sh -t $t -c $c -d $d -R $R" > ./results/$dir_date/swarm-mm-wrk-compose-$output_name
		...
		if [ $? -ne 0 ]; then
			...
			if [ $horizontal -eq 0 ]; then
				...
				nginx_ip=10.10.1.$(($(ssh stvdp@$node_name_nginx "hostname | cut -d'.' -f1 | sed 's/node//'")+1))
				ssh -n stvdp@$node_name_nginx "cd DeathStarBench/socialNetwork/ &&  python3 scripts/init_social_graph.py"  | grep Failed | wc -l
				while [ $? -ne 0 ]; do
					echo "socialNetwork still needs to be updated..."
					ssh -n stvdp@$node_name_nginx "cd DeathStarBench/socialNetwork/ &&  python3 scripts/init_social_graph.py"  | grep Failed | wc -l
				done
			elif [ $horizontal -eq 1 ]; then
				node_name_nginx=$(ssh $manager "docker service ps social-network_nginx-thrift | awk 'FNR == 2 {print \$4}'")
		...
		ssh -n $remote "cd DeathStarBench/socialNetwork/wrk2 && export nginx_ip=$nginx_ip && ./workload-mixed.sh -t $t -c $c -d $d -R $R" > ./results/$dir_date/swarm-sn-wrk-mixed-$output_name
